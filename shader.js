...
(function () {
  ...
  const fragmentShaderSource = `
    ...
    void main() {
      ...
      // COLOR COMPOSITION
      vec3 finalColor = mix(COLOR_BG, COLOR_DARK, uv.y * 0.6 + 0.2);
      finalColor += COLOR_GOLD * beam1 * beam1_fade * 0.65 * u_glow;
      finalColor += COLOR_BLUE * beam2 * beam2_fade * 0.70 * u_glow;
      finalColor += COLOR_CUARZO * reflection_intersection * 0.25 * u_glow;
      finalColor += COLOR_GOLD * mouse_glint * u_glow;

      float vignette = 1.0 - length(uv - 0.5) * 0.45;
      finalColor *= clamp(vignette, 0.35, 1.0);

      // Anti-Overexposure Tone Compression
      finalColor = finalColor / (vec3(1.0) + finalColor * 0.35);

      if (u_grain > 0.5) {
        float grain = (hash12(gl_FragCoord.xy + fract(u_time)) - 0.5) * 0.025;
        finalColor += grain;
      }
